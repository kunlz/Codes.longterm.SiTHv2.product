    ...
    for i=length(axBp):-1:1
        ...
        for j=1:length(axBp(i).XData)
            ...
            if ~isempty(tjDataY)
                ...
                scatter(ax,repmat(tjDataX,[sum(outliBool),1]),outli,25,'filled',...
                    'CData',Tm.(theme.AxesTheme).EdgeColor,'MarkerEdgeColor','none')
                
                tjDataY(outliBool)=[];
                plot(ax,[tjDataX,tjDataX],[max(tjDataY),min(tjDataY)],'LineWidth',1,...
    ...
    for i=length(axBp):-1:1
        ...
        for j=1:length(cgX)
            ...
            if ~isempty(tjDataY)
                ...
                scatter(ax,repmat(tjDataX,[length(outli),1]),outli,25,'filled',...
                    'CData',Tm.(theme.AxesTheme).EdgeColor,'MarkerEdgeColor','none')
