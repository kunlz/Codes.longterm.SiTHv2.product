...
% Modified at 2020.06.18 at ITPCAS, Beijing

% Note : need <m_map> toolbox, it can be downloaded at :
...
%      'Gall-Peters'                   %15  Global, Equatorial region
...
% Main Function
function [ch,cmm] = plotmap(data, projection, varargin)
% function input:
% ---------------
% projection :: the projection used in plotting map
% data       :: the candidate data need be ploted
% varargin   :: properties in mapping
% --------
% Example:
% [cmm, h] = plotmap(Rate, 'Robinson', 'res', 0.5,...
%                    'latlim', [-90,90], 'lonlim', [-179,179],...
%                    'krange', [1,1000], 'cmap', 'cmap10',...
%                    'labelname', 'Surface Runoff (mm year{^{-1}})',...
%                    'fontsize', 12);
...
% Get the spa_res if for global application
tmp = strncmpi(varargin, 'globres', 7);
if any(tmp)
    glob = true;
    spa_res = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
...
% Get the spa_res if for global application
tmp = strncmpi(varargin, 'ytics', 5);
if any(tmp)
    yxy = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% Get the latitude and longitude cell for the regional applicaiton
tmp = strncmpi(varargin, 'latlon', 6);
if any(tmp)
    latlon = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% Get the colormap name based on brew
tmp = strncmpi(varargin, 'colormap', 8);
if any(tmp)
    cmpname = varargin{find(tmp) + 1}; % '*RdYlBu'
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% figure title
tmp = strncmpi(varargin, 'title', 5);
if any(tmp)
    titlename = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% Get the latlim
tmp = strncmpi(varargin, 'latlim', 6);
if any(tmp)
    latlim = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% Get the lonlim
tmp = strncmpi(varargin, 'lonlim', 6);
if any(tmp)
    lonlim = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
end

% Get the range of colorbar, k
tmp = strncmpi(varargin, 'krange', 6);
if any(tmp)
    krange = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
else
    krange = [min(data(:)), max(data(:))];
end

% Get labelname
tmp = strncmpi(varargin, 'labelname', 7);
if any(tmp)
    labelname = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
else
    labelname = '';
...
% Get fontsize
tmp = strncmpi(varargin, 'fontsize', 7);
if any(tmp)
...
% Get the pvalue
tmp = strncmpi(varargin, 'pvalue', 6);
if any(tmp)
    cond_pv = true;
    pvalue = varargin{find(tmp) + 1};
    tmp(find(tmp) + 1) = 1;
    varargin = varargin(~tmp);
else
    cond_pv = false;
...
%     varargin = varargin(~tmp);
% else
%     cmap = m_colmap('diverging');
% end


% Get the colorbar ticks
% tmp = strncmpi(varargin, 'yxy', 3);
%
% if any(tmp)
%     yxtic = varargin{find(tmp) + 1};
% else
%     yxtic = a1:(a2-a1)/8:a2;
% end
...
if ~isempty(varaout)
    cond = false;
    assert(cond, 'Error: Additional input for this function ! ')
    return
end
...
% Create the coast line and fill it with lighten grey color
% m_coast('patch', [1 1 1], 'edgecolor', [0.1, 0.1, 0.1]);

% Plot data with its latitude and longitude data
chc = m_pcolor(LG11, LT11, data);
% m_coast('patch', [1 1 1], 'edgecolor', [0.1, 0.1, 0.1]);
XData = chc.XData;
YData = chc.YData;
...
% P-value section
if cond_pv
    % Plot the p-value (<0.05) with significance point
    mask = pvalue == 1;
    density = 100; % points density, it can be adjusted
    ...
    x = imresize(XData, sc); % Resize the grids with sc
    y = imresize(YData, sc); % Resize the grids with sc
    mask = imresize(mask, sc); % Resize the grids with sc
    plot(x(mask), y(mask), 'MarkerSize', 3, 'Marker', '.', ...
        'LineStyle', 'none', 'Color', [0.2 0.2 0.2,0.3]); % Marker size and color
end

% geo-grid
% m_grid('linewi', 0.5, 'tickdir', 'in', ... % set line
%     'FontName', 'Arial', 'FontSize', fontsize, ... % set the font
%     'backcolor', [245, 245, 245] ./ 255,... % set the background color
%     'linest', '-', ... % '-' | '--' | ':' | '-.' | 'none'
%     'gridcolor', [.6 .6 .6],...
%     'ticklen', 0.01, ...
%     'xtick',(-180:60:180), 'xticklabels',[],  ...
%     'ytick', (-90:30:90), 'yticklabels',[]);

m_grid('linewi', 1, 'tickdir', 'in', ... % set line
    'FontName', 'Arial', 'FontSize', fontsize, ... % set the font
    'backcolor', [255, 255, 255] ./ 255,... % set the background color
    'linest', '-', ... % '-' | '--' | ':' | '-.' | 'none'
    'gridcolor', [.6 .6 .6],...
    'ticklen', 0.01, ...
    'xtick',-120:60:120,  ... % (-120:60:120)
    'ytick', -90:45:90); % (-60:30:80)


m_gshhs('lc1','color',[0.2,0.2,0.2]); % high resolution coastline
% m_grid('linewi',0.5,'tickdir','in',... % set line
%     'FontName','Arial', 'FontSize',fontsize,... % set the font
%     'backcolor',[255, 255, 255] ./ 255); % set the background color

% replot the coastline in black
% m_coast('color',[0 0 0]);

% set title
set(get(gca,'title'),'String',titlename,...
    'FontName','Arial','FontSize',fontsize+1);

% set the range of colorbar, k
clim([krange(1) krange(length(krange))]);

% colorbar setting
ch = colorbar('southoutside','YTick',yxy,...
    'FontName', 'Arial', 'FontSize', fontsize, 'TickDirection','out',...
    'linewidth',1,'ticklength',0.005);

% set(h,'position',[.2 .2 .63 .03]) % [left, bottom, width, height]
% ax1 = gca;
% GcaPosition = ax1.Position;
% ax1Pos = ax1.Position;

% ax1.Position = GcaPosition;
% ax1.Position = ax1Pos;

% set the label of colorbar
set(get(ch, 'xlabel'), 'String', labelname, ...
    'FontName', 'Arial', 'FontSize', fontsize);

% set the 'log' or 'linear' for mapshow
set(gca,'colorscale','linear');

% set colormap
cmm = brewermap(49, cmpname); 
colormap(cmm)
